...
const JobDetailsPanel: React.FC<JobDetailsPanelProps> = ({ job, jobMetadata }) => {
  return (
    <aside className='w-120 shrink-0 border-l bg-white/60 backdrop-blur p-4 h-full'>
...
const JobDetailCenter: React.FC<JobDetailCenterProps> = ({
  ...
  return (
    <div className='flex flex-1 overflow-hidden rounded-xl border bg-white/70 shadow-sm h-full'>
      ...
      <TabsRail active={activeTab} onChange={onChangeTab} />
...
const ConsolePane: React.FC<ConsolePaneProps> = ({ title, lines }) => {
  ...
  return (
    <div className='min-h-0 flex-1 overflow-hidden h-full'>
      ...
      <div
        ref={scrollerRef}
        className='h-full overflow-y-auto bg-black text-neutral-100 font-mono text-[12px] leading-5'
      >
...
const ScriptPane: React.FC<ScriptPaneProps> = ({ script }) => {
  return (
    <div className='min-h-0 flex-1 overflow-hidden h-full'>
      ...
      <div className='h-full overflow-y-auto bg-neutral-50 text-neutral-800'>
...
const TabsRail: React.FC<TabsRailProps> = ({ active, onChange }) => {
  return (
    <nav
      aria-label='Output streams'
      className='w-14 shrink-0 border-l bg-white/70 backdrop-blur h-full'
    >
      <ul className='flex h-full flex-col items-stretch py-2'>
        {OUTPUT_TABS.map((t) => (
          <li key={t.id} className='px-1 py-1'>
            <button
              type='button'
              onClick={() => onChange(t.id)}
              data-active={active === t.id}
              className='group flex p-4 w-full items-center justify-center rounded-md border text-[10px] font-semibold tracking-wide bg-gray-100 hover:bg-gray-200 data-[active=true]:bg-gray-300'
              aria-pressed={active === t.id}
              aria-label={t.label}
              title={t.label}
            >
              <span className='[writing-mode:vertical-rl] rotate-180 leading-none'>{t.label}</span>
            </button>
          </li>
        ))}
      </ul>
